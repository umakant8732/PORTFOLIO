...
export default function About() {
  ...
  return (
    <section id="about" ref={containerRef} className="py-28 bg-[#0b0c0f] border-y border-zinc-900/60 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
          ...
          <div className="about-left lg:col-span-5 space-y-12">
            ...
            {/* Metrics Dashboard */}
            <div className="stats-grid grid grid-cols-3 gap-4 pt-6 border-t border-zinc-900/80">
              <div className="text-center p-3 rounded-lg bg-zinc-950/60 border border-zinc-900">
                <span className="block text-2xl sm:text-3xl font-bold text-white"><span className="stat-num-1">0</span>+</span>
                <span className="text-[10px] sm:text-xs text-zinc-500 uppercase tracking-wider mt-1 block">Modules Delivered</span>
              </div>
              <div className="text-center p-3 rounded-lg bg-zinc-950/60 border border-zinc-900">
                <span className="block text-2xl sm:text-3xl font-bold text-white"><span className="stat-num-2">0</span>%</span>
                <span className="text-[10px] sm:text-xs text-zinc-500 uppercase tracking-wider mt-1 block">API Optimization</span>
              </div>
              <div className="text-center p-3 rounded-lg bg-zinc-950/60 border border-zinc-900">
                <span className="block text-2xl sm:text-3xl font-bold text-white"><span className="stat-num-3">0</span>+</span>
                <span className="text-[10px] sm:text-xs text-zinc-500 uppercase tracking-wider mt-1 block">Connected APIs</span>
              </div>
            </div>
            ...
            <div className="space-y-4 pt-4">
              ...
              <div className="flex items-center gap-3 text-sm text-zinc-400">
                <GraduationCap className="w-5 h-5 text-zinc-600 shrink-0" />
          ...
          <div className="lg:col-span-7 timeline-container relative pl-8 pb-4">
            ...
            <div className="absolute left-3.5 top-0 bottom-0 w-0.5 bg-zinc-900" />
            <div 
              ref={timelineLineRef}
              className="absolute left-3.5 top-0 bottom-0 w-0.5 bg-zinc-600 origin-top transform scale-y-0" 
            />

            {/* Timeline Segment: Experience */}
            <div className="space-y-10 relative">
              <div className="flex items-center gap-2.5 mb-8 -ml-[45px] bg-[#0b0c0f] pr-4 py-1 z-10 w-fit">
                <div className="w-8 h-8 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center">
                  <Briefcase className="w-4 h-4 text-zinc-400" />
                </div>
                <h4 className="text-xs font-bold uppercase tracking-wider text-zinc-400">Experience History</h4>
              </div>

              {experiences.map((exp, index) => (
                <div key={index} className="timeline-card relative group">
                  {/* Bullet indicator */}
                  <div className="absolute -left-[41px] top-4 w-4 h-4 rounded-full bg-[#0b0c0f] border-2 border-zinc-700 group-hover:border-white transition-colors z-20" />
                  
                  <div className="glass-card p-6 rounded-xl">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                      <div>
                        <h5 className="text-base sm:text-lg font-bold text-white">{exp.title}</h5>
                        <p className="text-xs sm:text-sm text-zinc-400 font-medium">{exp.company} &bull; <span className="font-normal text-zinc-500">{exp.location}</span></p>
                      </div>
                      <span className="inline-flex items-center gap-1.5 text-xs text-zinc-400 bg-zinc-900 border border-zinc-800 px-2.5 py-1 rounded w-fit h-fit">
                    ...
                    <ul className="space-y-2.5">
                      {exp.bullets.map((bullet, idx) => (
                        <li key={idx} className="text-xs sm:text-sm text-zinc-400 flex items-start gap-2">
                          <CheckSquare className="w-4 h-4 text-zinc-600 mt-0.5 shrink-0" />
